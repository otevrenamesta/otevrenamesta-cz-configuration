# local copy of the parsoid module which is broken in nixos-19.09
# can be removed after it's fixed in upstream: https://github.com/NixOS/nixpkgs/pull/80998
# packages/parsoid/ can also be removed then
...
let
  ...
  cfg = config.services.parsoid_;

  parsoid = (import ../packages/parsoid { inherit pkgs; }).parsoid;
...
{
  imports = [
    (mkRemovedOptionModule [ "services" "parsoid_" "interwikis" ] "Use services.parsoid.wikis instead")
  ];
